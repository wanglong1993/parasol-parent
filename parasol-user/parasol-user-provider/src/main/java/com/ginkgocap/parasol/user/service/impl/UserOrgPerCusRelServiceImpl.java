...
public class UserOrgPerCusRelServiceImpl extends BaseService<UserOrgPerCusRel> implements UserOrgPerCusRelService {
	private static int error_uesrId_is_null = 1000;
	...
	public Long createUserOrgPerCusRel(UserOrgPerCusRel UserOrgPerCusRel) throws UserOrgPerCusRelServiceException {
		try {
			if (UserOrgPerCusRel.getUserId()==null ||UserOrgPerCusRel.getUserId()<=0l) throw new UserOrgPerCusRelServiceException(error_uesrId_is_null,"userId is null or empty.");
	...
	public boolean updateName(Long friendId,String name) throws UserOrgPerCusRelServiceException {
		try {
